...
- (IBAction)ObtainScan:(id)sender {
    ...
    if ([self.AmtTextField.text integerValue] < 1) {
        [Common showMsgBox:nil msg:@"收款金额大于1元" parentCtrl:self];
    }else if ([self.AmtTextField.text integerValue] > 50000){
        [Common showMsgBox:nil msg:@"收款金额超限" parentCtrl:self];
    }
    else{
